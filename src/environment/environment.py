...
from src.environment.building_simulation import BuildingSimulation
...
class Environment(gym.Env):
    ...
    def __init__(
        ...
        self.data_profile_length = len(
            self.building_simulation.solar_generation_profile
        )

        assert (
            self.max_timesteps + self.num_forecasting_steps <= self.data_profile_length
    ...
    def reset(
        self, seed: Union[int, None] = None, options: Any = None
        ...
        if self.randomize_start_time_step:
            latest_possible_start_time_step = (
                self.data_profile_length
                - self.max_timesteps
                - self.num_forecasting_steps
            )
            self.building_simulation.start_index = int(
                np.random.uniform(0, latest_possible_start_time_step)
            )
        return self.get_observation(), {}

    def step(
        self, action: Union[float, Iterable[float]]
    ...
    def get_observation(self) -> np.ndarray:
        current_index = (
            self.building_simulation.start_index + self.building_simulation.step_count
        )
        sim = self.building_simulation
        electric_load_forecast = sim.electricity_load_profile[
            current_index : current_index + self.num_forecasting_steps
        ]
        solar_gen_forecast = sim.solar_generation_profile[
            current_index : current_index + self.num_forecasting_steps
        ]
        energy_price_forecast = sim.electricity_price[
            current_index : current_index + self.num_forecasting_steps
        ]
        ...
        return np.concatenate(
            (
                [self.building_simulation.battery.state_of_charge],  # type: ignore
    ...
    @staticmethod
    def _randomize_forecast(
        forecast: np.ndarray,
        standard_deviation_start: float = 0.2,
        standard_deviation_end: float = 1.0,
    ) -> np.ndarray:

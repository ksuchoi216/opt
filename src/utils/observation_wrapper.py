...
class ObservationWrapper(gymnasium.Wrapper):
    ...
    def __init__(self, env, forecast_length):
        ...
        self.observation_space = gymnasium.spaces.Box(
            shape=(original_observation_space_length - forecast_length,),
            low=-np.inf,
            high=np.inf,
            dtype=np.float32,
        )
    ...
    def convert_observation(self, obs):
        ...
        generation_forecast = obs[
            self.forecast_length + 1 : 2 * self.forecast_length + 1
        ]
        price_forecast = obs[
            2 * self.forecast_length + 1 : 3 * self.forecast_length + 1
        ]
        soc = obs[0]
        return np.concatenate(
            ([soc], load_forecast - generation_forecast, price_forecast), axis=0
        )

...
class Simulation(object):
    ...
    def __init__(self, data, battery, site_id):
    ...
    def run(self):
        ...
        for current_time, timestep in tqdm(
            self.data.iterrows(), total=self.data.shape[0], desc=" > > > > timesteps\t"
        ):
    ...
    def simulate_timestep(self, battery_controller, current_time, timestep):
        ...
        grid_energy, battery_energy_change = self.simulate_battery_charge(
        ...
        price_without_battery = (
            timestep.price_buy_00
            if grid_energy_without_battery >= 0
            else timestep.price_sell_00
        )

        # calculate spending based on price per kWh and energy per Wh
        self.money_spent += grid_energy * (price / 1000.0)
        self.money_spent_without_battery += grid_energy_without_battery * (
            price_without_battery / 1000.0
        )
    ...
    def simulate_battery_charge(
        ...
        target_energy_change = (
            proposed_state_of_charge - initial_state_of_charge
        ...
        if target_energy_change >= 0:
            ...
            target_charging_power = target_energy_change / ((15.0 / 60.0) * efficiency)
        else:
            efficiency = self.battery.discharging_efficiency
            target_charging_power = target_energy_change * efficiency / (15.0 / 60.0)

        # actual power is bounded by the properties of the battery
        actual_charging_power = np.clip(
        ...
        if actual_charging_power >= 0:
            actual_energy_change = actual_charging_power * (15.0 / 60.0) * efficiency
        else:
            actual_energy_change = actual_charging_power * (15.0 / 60.0) / efficiency

        # what we need from the grid = (the power put into the battery + the consumption) - what is available from pv
        grid_energy = (
            actual_charging_power * (15.0 / 60.0) + actual_consumption
        ) - actual_pv
...
if __name__ == "__main__":
    ...
    for site_id, parameters in tqdm(
        metadata.iterrows(), desc="sites\t\t\t", total=metadata.shape[0]
    ):
        site_data_path = data_dir / "submit" / f"{site_id}.csv"

        if site_data_path.exists():
            site_data = pd.read_csv(
                site_data_path, parse_dates=["timestamp"], index_col="timestamp"
            )
            ...
            for batt_id in tqdm([1, 2], desc=" > batteries \t\t"):
                ...
                batt = Battery(
                    capacity=parameters[f"Battery_{batt_id}_Capacity"] * 1000,
    ...
    results_df = results_df[
        ["site_id", "battery_id", "period_id", "money_spent", "money_no_batt", "score"]
    ]
